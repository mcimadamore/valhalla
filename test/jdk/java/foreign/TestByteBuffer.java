...
public class TestByteBuffer {
    ...
    public void testMappedSegment() throws Throwable {
        ...
        try (MemorySegment segment = MemorySegment.mapFromPath(f.toPath(), 0L, tuples.byteSize(), FileChannel.MapMode.READ_WRITE)) {
            initTuples(segment, tuples.elementCount().getAsLong());
            jdk.incubator.foreign.MappedMemorySegments.force(segment);
        }
    ...
    public static Object[][] bufferSources() {
        ...
        Predicate<MemorySegment> nativeTest = segment -> ((MemorySegmentImpl)segment).unsafeGetBase() == null &&
                segment.fileDescriptor().isEmpty();
        Predicate<MemorySegment> mappedTest = segment -> ((MemorySegmentImpl)segment).unsafeGetBase() == null &&
                segment.fileDescriptor().isPresent();
    ...
    enum MappedSegmentOp {
        LOAD(jdk.incubator.foreign.MappedMemorySegments::load),
        UNLOAD(jdk.incubator.foreign.MappedMemorySegments::unload),
        IS_LOADED(jdk.incubator.foreign.MappedMemorySegments::isLoaded),
        FORCE(jdk.incubator.foreign.MappedMemorySegments::force),
